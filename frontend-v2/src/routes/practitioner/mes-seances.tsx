...
export const Route = createFileRoute("/practitioner/mes-seances")({
...
function MesSeancesPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      api.getSessions({ praticien_id: user?.id, page, size: 20 }),
    enabled: !!user?.id,
  });
  ...
  return (
    <div className="page-container space-y-4 sm:space-y-6">
      <div>
        <h1 className="heading-2">Mes seances</h1>
      ...
      <Card>
        <CardHeader className="pb-3">
          <CardTitle className="text-base">Historique de mes seances</CardTitle>
        </CardHeader>
        <CardContent>
          {isLoading ? (
