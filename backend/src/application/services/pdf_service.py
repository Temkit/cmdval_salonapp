...
class PDFService:
    ...
    def generate_consent_form(self, patient_data: dict) -> bytes:
        """Generate a consent form PDF for laser treatment.

        Args:
            patient_data: Dictionary with keys nom, prenom, code_carte, etc.
        ...
        self._build_header(elements, "Formulaire de Consentement")
        ...
        elements.append(
            Paragraph(
                ...
                "le traitement par laser propose par le centre Optiskin :".format(
                    prenom=patient_data.get("prenom", ""),
                    nom=patient_data.get("nom", ""),
                ),
                self._styles["BodyFR"],
        ...
        for heading, text in clauses:
            ...
            elements.append(Paragraph(text, self._styles["BodyFR"]))
        ...
        today = date.today().strftime("%d/%m/%Y")
        elements.append(Spacer(1, 4 * mm))
        elements.append(Paragraph(f"Fait a ________________, le {today}", self._styles["BodyFR"]))

        self._build_signature_block(elements)

        return self._render_pdf(elements)

    def generate_clinic_rules(self, patient_data: dict) -> bytes:
        """Generate the clinic laser safety rules PDF.
        ...
        self._build_header(elements, "Reglement Interieur - Securite Laser")
        elements.append(self._build_patient_info_table(patient_data))
        elements.append(Spacer(1, 8 * mm))
        ...
        sections = [
            (
                "Article 1 - Securite laser",
                [
                    "Le port de lunettes de protection est obligatoire pendant "
                    "toute la duree du traitement, tant pour le patient que pour le praticien.",
                    "Il est strictement interdit de retirer les lunettes de protection "
                    "pendant que le laser est en fonctionnement.",
                    "Tout dysfonctionnement ou sensation anormale doit etre signale "
                    "immediatement au praticien.",
                ],
            ),
            (
                "Article 2 - Preparation avant seance",
                [
                    "La zone a traiter doit etre rasee 24 a 48 heures avant la seance.",
                    "Aucun produit cosmetique (creme, deodorant, parfum) ne doit etre "
                    "applique sur la zone a traiter le jour de la seance.",
                    "Toute exposition au soleil ou aux UV (cabine de bronzage) doit "
                    "etre evitee pendant les 4 semaines precedant la seance.",
                ],
            ),
            (
                "Article 3 - Deroulement de la seance",
                [
                    "Le patient doit arriver a l'heure de son rendez-vous. "
                    "Un retard de plus de 15 minutes pourra entrainer l'annulation "
                    "de la seance.",
                    "Le patient doit informer le praticien de tout changement de "
                    "son etat de sante, de tout nouveau traitement medicamenteux "
                    "ou de toute grossesse.",
                    "Le praticien se reserve le droit de refuser ou de reporter "
                    "une seance si les conditions de securite ne sont pas reunies.",
                ],
            ),
            (
                "Article 4 - Apres la seance",
                [
                    "Appliquer la creme apaisante fournie selon les indications du praticien.",
                    "Eviter toute exposition au soleil ou aux UV pendant les 4 semaines "
                    "suivant la seance.",
            ...
            (
                ...
                [
                    ...
                    "En cas d'annulation tardive ou d'absence sans prevenir, la seance "
                    "pourra etre consideree comme effectuee.",
                ],
            ),
            (
                "Article 6 - Responsabilite",
                [
                    "Le centre Optiskin decline toute responsabilite en cas de "
                    "non-respect des consignes par le patient.",
                    "Le patient s'engage a fournir des informations exactes et "
                    "completes sur son etat de sante.",
                ],
            ),
        ]
    ...
    def generate_precautions(self, patient_data: dict) -> bytes:
        """Generate the pre/post treatment precautions PDF.
        ...
        self._build_header(elements, "Precautions Pre et Post-Traitement Laser")
        elements.append(self._build_patient_info_table(patient_data))
        elements.append(Spacer(1, 8 * mm))
        ...
        # Pre-treatment
        elements.append(Paragraph("AVANT LE TRAITEMENT", self._styles["SectionHeading"]))

        pre_items = [
            "Eviter toute exposition au soleil et aux UV (cabines de bronzage) "
            "pendant au moins 4 semaines avant la seance.",
            "Ne pas appliquer de produits auto-bronzants pendant les 2 semaines "
            "precedant la seance.",
            "Raser la zone a traiter 24 a 48 heures avant le rendez-vous. "
            "Ne pas epiler a la cire, a la pince ou a l'epilateur electrique.",
            "Ne pas appliquer de creme, deodorant, parfum ou maquillage "
            "sur la zone le jour de la seance.",
            "Signaler au praticien tout traitement medicamenteux en cours, "
            "en particulier les medicaments photosensibilisants "
            "(antibiotiques, anti-inflammatoires, retinoides...).",
            "Informer le praticien de toute grossesse ou suspicion de grossesse.",
            "Signaler tout antecedent de cicatrisation anormale "
            "(cicatrices cheloides ou hypertrophiques).",
        ]
        for item in pre_items:
            elements.append(Paragraph(f"&bull; {item}", self._styles["BodyFR"]))

        # Post-treatment
        elements.append(Paragraph("APRES LE TRAITEMENT", self._styles["SectionHeading"]))

        post_items = [
            "Appliquer la creme apaisante recommandee par le praticien "
            "pendant les 3 a 5 jours suivant la seance.",
            "Eviter toute exposition au soleil et aux UV pendant au moins "
            "4 semaines apres la seance. Utiliser un ecran solaire SPF 50+ "
            "si l'exposition est inevitable.",
            "Ne pas gratter, frotter ou exfolier la zone traitee pendant au moins 1 semaine.",
            "Eviter les bains chauds, saunas, hammams et piscines "
            "pendant 48 heures apres la seance.",
            "Eviter l'activite sportive intense pendant 24 a 48 heures apres le traitement.",
            "Les poils traites tomberont naturellement dans les 10 a 21 jours "
            "suivant la seance. Ne pas les arracher.",
            "En cas de rougeurs persistantes, de cloques, de croutes ou de "
            "toute reaction inhabituelle, contacter immediatement le centre.",
        ]
        ...
        warning_data = [
            [
                Paragraph(
                    "<b>IMPORTANT :</b> Le non-respect de ces precautions peut "
